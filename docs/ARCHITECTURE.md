...
- **rules/analyzers** apply the heuristics shipped in `meetingcost/core.py`.
- **score** ranks by severity.
- **MCP server** (`meetingcost mcp`) exposes `scan` for Cognis.Studio agents.
